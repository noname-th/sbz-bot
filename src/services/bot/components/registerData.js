...
module.exports = {
    ...
    data: {
        ...
        embed: (role) => {
            return new EmbedBuilder()
                .setTitle('ลงทะเบียน')
                .setDescription(`คลิกปุ่มด้านล่างเพื่อลงทะเบียน\nซึ่งจะปลดล็อกห้องสำหรับ ${role}`)
                .setColor(role.color)
        },
    ...
    run: async (interaction) => {
        ...
        if (interaction.member.roles.cache.has(roleId)) {
            await interaction.reply({ content: '❌ คุณมีบทบาทนี้อยู่แล้ว!', ephemeral: true });
            return;
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0x82CD47)
            .setThumbnail(`https://static.vecteezy.com/system/resources/previews/009/973/338/non_2x/tick-check-mark-icon-sign-symbol-design-free-png.png`)
            .setTimestamp()

        // add role for registered user
        await interaction.member.roles.add(role);
        await interaction.reply({ content: '✅ ลงทะเบียนเสร็จสิ้น', ephemeral: true });
